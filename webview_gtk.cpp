...
typedef GdkWindow *(*gdk_x11_window_foreign_new_for_display_ptr)(
    GdkDisplay *display, void *xwindow);

gdk_x11_lookup_xdisplay_ptr webview_gdk_x11_lookup_xdisplay = nullptr;
gdk_x11_window_foreign_new_for_display_ptr
    webview_gdk_x11_window_foreign_new_for_display = nullptr;
...
public:
  ...
  static gboolean msg_callback(WebKitWebView *p_web_view,
                               WebKitUserMessage *p_message,
                               gpointer p_user_data) {
    WebViewOverlayImplementation *impl =
        (WebViewOverlayImplementation *)p_user_data;
    if (impl->control != nullptr) {
      impl->control->emit_signal(
          "callback", String::utf8(webkit_user_message_get_name(p_message)));
    }
    return TRUE;
  }

  static void load_callback(WebKitWebView *p_web_view,
                            WebKitLoadEvent p_load_event,
                            gpointer p_user_data) {
    WebViewOverlayImplementation *impl =
        (WebViewOverlayImplementation *)p_user_data;
    if (impl->control != nullptr) {
  ...
  static void insec_callback(WebKitWebView *p_web_view,
                             WebKitInsecureContentEvent p_event,
                             gpointer p_user_data) {
    WebViewOverlayImplementation *impl =
        (WebViewOverlayImplementation *)p_user_data;
    impl->secure = false;
  ...
  static void snapshot_callback(GObject *p_source_object, GAsyncResult *p_res,
                                gpointer p_user_data) {
    WebViewOverlayImplementation *impl =
        (WebViewOverlayImplementation *)p_user_data;
    ...
    cairo_surface_t *surf = webkit_web_view_get_snapshot_finish(
        WEBKIT_WEB_VIEW(impl->view), p_res, &error);
    ...
    impl->control->emit_signal(
        "snapshot_ready",
        memnew(Image(w, h, false, Image::FORMAT_RGBA8, imgdata)));
...
WebViewOverlay::~WebViewOverlay() { memdelete(data); }
...
void WebViewOverlay::_notification(int p_what) {
  ...
  case NOTIFICATION_INTERNAL_PROCESS: {
    if (!Engine::get_singleton()->is_editor_hint() && (data->view == nullptr) &&
      ...
      if (data->container == nullptr) {
        // void *xwindow =
        //     OS::get_singleton()->get_native_handle(OS::WINDOW_HANDLE);
        // void *xdisplay =
        //     OS::get_singleton()->get_native_handle(OS::DISPLAY_HANDLE);
        ...
        GdkWindow *window = webview_gdk_x11_window_foreign_new_for_display(
            webview_gdk_x11_lookup_xdisplay(xdisplay), xwindow);
      ...
      webkit_user_content_manager_add_script(
          manager,
          webkit_user_script_new("function "
                                 "webviewMessage(s){window.webkit."
                                 "messageHandlers.callback.postMessage(s);}",
                                 WEBKIT_USER_CONTENT_INJECT_TOP_FRAME,
                                 WEBKIT_USER_SCRIPT_INJECT_AT_DOCUMENT_START,
                                 nullptr, nullptr));
      webkit_user_content_manager_register_script_message_handler(manager,
                                                                  "callback");
      g_signal_connect_data(G_OBJECT(webview), "user-message-received",
                            G_CALLBACK(data->msg_callback), data, nullptr,
                            (GConnectFlags)0);
      g_signal_connect_data(G_OBJECT(webview), "load-changed",
                            G_CALLBACK(data->load_callback), data, nullptr,
                            (GConnectFlags)0);
      g_signal_connect_data(G_OBJECT(webview), "insecure-content-detected",
                            G_CALLBACK(data->insec_callback), data, nullptr,
                            (GConnectFlags)0);

      float sc = DisplayServer::get_singleton()->screen_get_max_scale();
      Rect2i rect = get_window_rect();
      gtk_widget_set_size_request(GTK_WIDGET(webview), rect.size.width / sc,
                                  rect.size.height / sc);
      gtk_fixed_put(GTK_FIXED(data->container), GTK_WIDGET(webview),
                    rect.position.x / sc, rect.position.y / sc);
  ...
  case NOTIFICATION_RESIZED: {
    if (data->view != nullptr) {
      float sc = DisplayServer::get_singleton()->screen_get_max_scale();
      Rect2i rect = get_window_rect();
      gtk_widget_set_size_request(GTK_WIDGET(data->view), rect.size.width / sc,
                                  rect.size.height / sc);
      gtk_fixed_move(GTK_FIXED(data->container), GTK_WIDGET(data->view),
                     rect.position.x / sc, rect.position.y / sc);
    }
  } break;
...
void WebViewOverlay::get_snapshot(int p_width) {
  ...
  webkit_web_view_get_snapshot(WEBKIT_WEB_VIEW(data->view),
                               WEBKIT_SNAPSHOT_REGION_VISIBLE,
                               WEBKIT_SNAPSHOT_OPTIONS_TRANSPARENT_BACKGROUND,
                               nullptr, data->snapshot_callback, data);
}
...
bool WebViewOverlay::get_no_background() const { return no_background; }

void WebViewOverlay::set_url(const String &p_url) {
  home_url = p_url;
  if (data->view != nullptr) {
    webkit_web_view_load_uri(WEBKIT_WEB_VIEW(data->view),
                             home_url.utf8().get_data());
  }
}
...
void WebViewOverlay::execute_java_script(const String &p_script) {
  ERR_FAIL_COND(data->view == nullptr);
  webkit_web_view_run_javascript(WEBKIT_WEB_VIEW(data->view),
                                 p_script.utf8().get_data(), nullptr, nullptr,
                                 nullptr);
}

void WebViewOverlay::load_string(const String &p_source) {
  ERR_FAIL_COND(data->view == nullptr);
  webkit_web_view_load_html(WEBKIT_WEB_VIEW(data->view),
...
bool WebViewOverlay::is_ready() const { return (data->view != nullptr); }
...
void WebViewOverlay::init() {
  ...
  webview_gdk_x11_lookup_xdisplay =
      (gdk_x11_lookup_xdisplay_ptr)dlsym(gdk3_lib, "gdk_x11_lookup_xdisplay");
  webview_gdk_x11_window_foreign_new_for_display =
      (gdk_x11_window_foreign_new_for_display_ptr)dlsym(
          gdk3_lib, "gdk_x11_window_foreign_new_for_display");
  // webview_gdk_rgba_parse = (gdk_rgba_parse_ptr)dlsym(gdk3_lib,
  // "gdk_rgba_parse");
  if (webview_gdk_x11_lookup_xdisplay == nullptr ||
      webview_gdk_x11_window_foreign_new_for_display ==
          nullptr /* || webview_gdk_rgba_parse == nullptr*/) {
    err_status = 2;
    return;
  }
