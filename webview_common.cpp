...
void WebViewOverlay::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("can_go_forward"),
			&WebViewOverlay::can_go_forward);
	...
	ClassDB::bind_method(D_METHOD("is_secure_content"),
			&WebViewOverlay::is_secure_content);
	...
	ClassDB::bind_method(D_METHOD("set_no_background", "no_background"),
			&WebViewOverlay::set_no_background);
	ClassDB::bind_method(D_METHOD("get_no_background"),
			&WebViewOverlay::get_no_background);
	...
	ClassDB::bind_method(D_METHOD("set_user_agent", "user_agent"),
			&WebViewOverlay::set_user_agent);
	ClassDB::bind_method(D_METHOD("get_user_agent"),
			&WebViewOverlay::get_user_agent);

	ClassDB::bind_method(D_METHOD("get_zoom_level"),
			&WebViewOverlay::get_zoom_level);
	ClassDB::bind_method(D_METHOD("set_zoom_level", "zoom"),
			&WebViewOverlay::set_zoom_level);

	ClassDB::bind_method(D_METHOD("load_string", "source"),
			&WebViewOverlay::load_string);
	ClassDB::bind_method(D_METHOD("execute_java_script", "script"),
			&WebViewOverlay::execute_java_script);

	ClassDB::bind_method(D_METHOD("get_snapshot", "width"),
			&WebViewOverlay::get_snapshot);

	ClassDB::bind_method(D_METHOD("get_title"), &WebViewOverlay::get_title);

	ADD_PROPERTY(PropertyInfo(Variant::BOOL, "no_background"),
			"set_no_background", "get_no_background");
	ADD_PROPERTY(PropertyInfo(Variant::STRING, "url"), "set_url", "get_url");
	ADD_PROPERTY(PropertyInfo(Variant::STRING, "user_agent"), "set_user_agent",
			"get_user_agent");
	ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "zoom_level"), "set_zoom_level",
			"get_zoom_level");
...
void WebViewOverlay::_draw_placeholder() {
	...
	font->draw_string(get_canvas_item(),
			Vector2((size.x - url_size.x) / 2, 100 + url_size.y) +
					Vector2(1, 1),
			home_url, HORIZONTAL_ALIGNMENT_LEFT, -1, font_size, Color(0, 0, 0, 0.5));
	font->draw_string(get_canvas_item(),
			Vector2((size.x - url_size.x) / 2, 100 + url_size.y) +
					Vector2(-1, 1),
			home_url, HORIZONTAL_ALIGNMENT_LEFT, -1, font_size, Color(0, 0, 0, 0.5));
	...
	font->draw_string(get_canvas_item(),
			Vector2((size.x - url_size.x) / 2, 100 + url_size.y),
			home_url, HORIZONTAL_ALIGNMENT_LEFT, -1, font_size, Color(1, 1, 1));
}
...
void WebViewOverlay::_draw_error(const String &p_error) {
	...
	font->draw_string(get_canvas_item(),
			Vector2((size.x - ti_size.x) / 2, 100 + ti_size.y) +
					Vector2(1, 1),
			err_ti, HORIZONTAL_ALIGNMENT_LEFT, -1, font_size, Color(0, 0, 0, 0.5));
	font->draw_string(get_canvas_item(),
			Vector2((size.x - ti_size.x) / 2, 100 + ti_size.y) +
					Vector2(-1, 1),
			err_ti, HORIZONTAL_ALIGNMENT_LEFT, -1, font_size, Color(0, 0, 0, 0.5));
	...
	font->draw_string(get_canvas_item(),
			...
			err_ti, HORIZONTAL_ALIGNMENT_LEFT, -1, font_size, Color(0, 0, 0, 0.5));
	font->draw_string(get_canvas_item(),
			Vector2((size.x - ti_size.x) / 2, 100 + ti_size.y), err_ti,
			HORIZONTAL_ALIGNMENT_LEFT, -1, font_size, Color(1, 0, 0));

	font->draw_string(
			get_canvas_item(),
			Vector2((size.x - er_size.x) / 2, 100 + er_size.y + 10 + ti_size.y) +
					Vector2(1, 1),
			p_error, HORIZONTAL_ALIGNMENT_LEFT, -1, font_size, Color(0, 0, 0, 0.5));
	font->draw_string(
			get_canvas_item(),
			Vector2((size.x - er_size.x) / 2, 100 + er_size.y + 10 + ti_size.y) +
					Vector2(-1, 1),
	...
	font->draw_string(
			get_canvas_item(),
			Vector2((size.x - er_size.x) / 2, 100 + er_size.y + 10 + ti_size.y),
			p_error, HORIZONTAL_ALIGNMENT_LEFT, -1, font_size, Color(1, 0, 0));
}
